...
Отдельный сайт в репозитории: каталог `personal-finance/`. Выписки, разнесение переводов без статьи, опциональный Telegram в личку. **Не использует** Mango, группы SOCO и `daily_pipeline`.
