...
Имена файлов **не переименовывать**. Период и «что новее» — по датам операций внутри PDF/CSV/Excel. Повторная загрузка того же периода не двоит строки, но **перепроверяет статьи**. PDF Альфа: копилка и внутрибанковские переводы → «между своими» без очереди; HOLD (ещё в обработке) входят в расходы. После каждой загрузки касса сверяет доходы/расходы с шапкой выписки («Поступления» / «Расходы») — на сайте должно совпасть. Подтянуть папку: кнопка на сайте или `python -m app.pull`.

Очередь переводов: «Разобрать позже» кладёт всю очередь на вкладку **Разобрать позже**; **Зачислить все доходы** — все плюсовые из очереди в статью «Доходы»; «Оставить неразмеченным» → раздел **Переводы без разметки**. По нажатию на статью, «Доходы» или «Расходы» открывается список операций; операцию можно открыть и сменить название статьи, **создать новую**, **переименовать статью** для всех таких операций, **отнести все с этим MCC** или **добавить в статью все с таким описанием** (кнопка на списке статьи и на карточке; без учёта регистра; плюсы и минусы отдельно; забирает и «между своими», например копилку; правило запоминается для следующих выписок). **Сальдо счёта** = все плюсы минус все минусы по выписке (переводы между своими и неразобранные тоже входят). Нижняя панель вкладок **без** `transform: translateX(-50%)` — на iPhone иначе зона тапа не совпадает с кнопкой.
