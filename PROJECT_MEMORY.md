...
Очередь переводов: «Разобрать позже»; вкладка **Неразобранные** (красная, пока есть очередь); «Оставить неразмеченным» → раздел **Переводы без разметки** (в сальдо входят, в обычные статьи нет).
