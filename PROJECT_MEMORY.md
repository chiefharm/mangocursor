...
Очередь переводов: «Разобрать позже» кладёт всю очередь на вкладку **Разобрать позже**; **Зачислить все доходы** — все плюсовые из очереди в статью «Доходы»; «Оставить неразмеченным» → раздел **Переводы без разметки**. По нажатию на статью, «Доходы» или «Расходы» открывается список операций.
