...
"${SCP[@]}" "$APP/deploy/"*.sh "$APP/deploy/"*.service "$HOST:$REMOTE_DIR/deploy/"
"${SSH[@]}" "$HOST" "chmod +x $REMOTE_DIR/deploy/*.sh; sed -i 's/\r\$//' $REMOTE_DIR/deploy/*.sh $REMOTE_DIR/deploy/*.service"

echo "==> install on server"
"${SSH[@]}" "$HOST" "FINANCE_SKIP_FETCH=1 DEST=$REMOTE_DIR FINANCE_PORT=$PORT bash $REMOTE_DIR/deploy/install_on_vps.sh"

echo
echo "Откройте на iPhone: http://$VPS_IP:$PORT"
